...
BoundSession bind_session(const fs::path& host_input,const fs::path& activation_path,const fs::path& model_input,int lease_fd,const fs::path& config_path) {
    ...
    const auto descriptor_path=regular(host,"provider/runtime-host-v2.json"); const auto descriptor=read_json(descriptor_path); const auto descriptor_sha256=expected.at("descriptorSha256").get<std::string>(); if(sha256_file_incremental_apple(descriptor_path)!=descriptor_sha256||descriptor.at("schemaVersion")!=2||descriptor.at("hostPackageId")!=expected.at("hostPackageId")||descriptor.at("providerId")!=expected.at("providerId")) throw std::runtime_error("host-mismatch");
    const auto activation_sha256=config.at("activationSha256").get<std::string>(); auto activation=read_json(activation_path); if(sha256_file_incremental_apple(activation_path)!=activation_sha256) throw std::runtime_error("activation-digest"); bind_activation(config,expected,activation);
...
void verify_complete_manifest(const BoundSession& session) {
    ...
    for(const auto& record:records) { exact(record,{"path","role","sizeBytes","sha256","mode"}); const auto relative_path=record.at("path").get<std::string>(); if(!valid_relative(relative_path)||(!previous.empty()&&relative_path<=previous)||record.at("mode")!="read-only"||!valid_sha(record.at("sha256"))) throw std::runtime_error("invalid-model-record"); const auto file_sha256=record.at("sha256").get<std::string>(); previous=relative_path; const auto file=session.resolve_model(relative_path); if(!read_only_single_link(file)||fs::file_size(file)!=record.at("sizeBytes").get<std::uintmax_t>()||sha256_file_incremental_apple(file)!=file_sha256) throw std::runtime_error("model-file-mismatch"); expected.insert(relative_path); rows.push_back({relative_path,record.at("sizeBytes"),record.at("sha256")}); }
    const auto tree_sha256=session.activation.at("model").at("treeSha256").get<std::string>(); std::set<std::string> actual; for(const auto& item:fs::recursive_directory_iterator(session.model_root)) { if(item.is_symlink()) throw std::runtime_error("model-link"); if(item.is_regular_file()) actual.insert(fs::relative(item.path(),session.model_root).generic_string()); } if(actual!=expected||sha256_bytes(rows.dump()+"\n")!=tree_sha256) throw std::runtime_error("model-closure-mismatch");
}
