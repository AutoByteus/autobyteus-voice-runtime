...
| `IR-041`    | Code Reviewer / `code-review-report.md` / `CRR-064`                | Remaining `CR-F-050`                                        | `Local Fix`           | `SR-025`; `ARCH-REV-025`; `CRR-063`, `CRR-064`; `API-REV-025`, `API-REV-027`; `DR-012`                                 | `Implementation Complete — Ready for Code Re-review` |
| `IR-042`    | Code Reviewer / `code-review-report.md` / `CRR-065`                | Remaining `CR-F-050`                                        | `Local Fix`           | `SR-025`; `ARCH-REV-025`; `CRR-063`–`CRR-065`; `API-REV-025`, `API-REV-027`; `DR-012`                                  | `Implementation Complete — Ready for Code Re-review` |
...
### IR-041 — Close executable workflow action indirection

- Triggering role, report path, and round: Code Reviewer; `/Users/normy/autobyteus_org/autobyteus-worktrees/voice-runtime-qualified-recovery/tickets/done/voice-input-runtime-reliability/code-review-report.md`; `CRR-064`, with evidence at `/Users/normy/autobyteus_org/autobyteus-worktrees/voice-runtime-qualified-recovery/tickets/done/voice-input-runtime-reliability/code-review-evidence/CRR-064-package-action-indirection-review.md` and `/Users/normy/autobyteus_org/autobyteus-worktrees/voice-runtime-qualified-recovery/tickets/done/voice-input-runtime-reliability/code-review-evidence/crr-064/SHA256SUMS.txt`.
- Triggering finding IDs: remaining `CR-F-050`; `CR-F-049` resolved in `CRR-064`.
- Classification: `Local Fix`.
- Prior authoritative result: `CRR-064` / `Fail — Local Fix`. Direct run-scalar package-manager closure and focused-renewal truth were correct, but an added executable `uses: actions/github-script@v7` step could perform the same undeclared npm install without entering the run-scalar scanner. API/E2E remained paused.
- Current authoritative result: `Implementation Complete — Ready for Code Re-review`.
- Related solution revision IDs: `SR-025`.
- Related architecture-review revision IDs: `ARCH-REV-025` Pass; no design reset required.
- Related code-review revision IDs: `CRR-063`, `CRR-064`; current re-review pending.
- Related API/E2E revision IDs: historical `API-REV-025`, hosted failure context `API-REV-027`; focused renewal pending.
- Related delivery revision IDs: `DR-012`.
- Why this implementation revision is recorded: the sole workflow-semantic authority validated direct `run:` package-manager commands but treated executable action identities and inputs as metadata. The supported maintained-main workflow executes those actions before construction, so their exact surface must be closed by the same authority.
- Approved behavior or requirement IDs affected: `BEH-004`, `BEH-007`, `BEH-013`; `R-014`, `R-018`, `R-024`, `R-029`, `R-030`; `AC-006`, `AC-010`, `AC-025`, `AC-035`, `AC-036`; confirmed `MP-AR-025B`. Runtime/provider/model/Protocol/archive/assets/desktop behavior remains unchanged.
- Implementation delta:
  - Extracted one private `workflow-executable-surface.mjs` helper under the public Host Package Input Contract facade. It retains exact package-manager command admission and now projects every executable workflow action step semantically by ordered action identity, step ID/condition, and a canonical input map.
  - The helper admits only the reviewed checkout, exact Node selector/cache, exact Go selector, and terminal audit upload actions with their exact inputs. Any added/replaced action, action-version/input/condition drift, unexpected action field, duplicate action property/input, quoted executable key, or noncanonical step surface rejects.
  - Added the reviewer-shaped `actions/github-script@v7` negative whose script invokes `npm install --ignore-scripts --no-save ajv@8.19.0`, plus approved-action input drift coverage. Non-executable workflow name and standalone comment metadata, including textual npm/uses references, still project byte-identically.
  - Bound the new private helper's ordinary-file identity in Host Source Closure 1 and added a direct closure assertion. No raw workflow hash, caller-side validator, second public authority, or projection fallback exists.
- Changed files or areas: `build/workflow-executable-surface.mjs`, `build/host-package-input-contract.mjs`, `build/host-source-closure.mjs`, `tests/build/host-package-input-contract.test.mjs`, `tests/build/host-source-closure.test.mjs`; source commit `447d34148500ef56aad94a62da215e7f3066e2b6` after IR-040 source `a66a7eeb604a94445070b7573abe5a5d6238efc1`.
- Local validation and result: combined focused contract/closure/release/admission tests passed `18/18`; `npm run check:release-pipeline` passed `22/22`; exact pinned-Go full `npm run check` passed source guards, `7/7` Python plus compileall, all Go/source/evidence checks, and `112/112` Node TAP tests with no failures or skips. Prettier, diff checks, staged commit checks, and source-size checks passed; public contract is 413 effective non-empty lines and private helper is 176.
- Next recipient or routing: `code_reviewer`.
- Remaining limitations or risks: implementation did not run API/E2E, construct/install production hosts, run offline clips or product qualification, compare Execution Closure 2, renew focused authorities, promote `R`, dispatch release, merge, tag, publish, or touch desktop/user state. API/E2E remains paused pending Source Pass; any later Execution Closure 2 mismatch routes to full qualification.

### IR-042 — Reject alternate workflow execution representations

- Triggering role, report path, and round: Code Reviewer; `/Users/normy/autobyteus_org/autobyteus-worktrees/voice-runtime-qualified-recovery/tickets/done/voice-input-runtime-reliability/code-review-report.md`; `CRR-065`, with evidence at `/Users/normy/autobyteus_org/autobyteus-worktrees/voice-runtime-qualified-recovery/tickets/done/voice-input-runtime-reliability/code-review-evidence/CRR-065-workflow-executable-resolution-review.md` and `/Users/normy/autobyteus_org/autobyteus-worktrees/voice-runtime-qualified-recovery/tickets/done/voice-input-runtime-reliability/code-review-evidence/crr-065/SHA256SUMS.txt`.
- Triggering finding IDs: remaining `CR-F-050`; `CR-F-049` remains resolved.
- Classification: `Local Fix`.
- Prior authoritative result: `CRR-065` / `Fail — Local Fix`. The canonical block-action probe rejected, but actionlint-valid flow-style `actions/github-script@v7` and custom-shell npm-install forms remained outside the line-oriented action enumeration and retained the unchanged install projection. API/E2E remained paused.
- Current authoritative result: `Implementation Complete — Ready for Code Re-review`.
- Related solution revision IDs: `SR-025`.
- Related architecture-review revision IDs: `ARCH-REV-025` Pass; no design reset required.
- Related code-review revision IDs: `CRR-063`, `CRR-064`, `CRR-065`; current re-review pending.
- Related API/E2E revision IDs: historical `API-REV-025`, hosted failure context `API-REV-027`; focused renewal pending.
- Related delivery revision IDs: `DR-012`.
- Why this implementation revision is recorded: the one private workflow parser recognized only canonical block-style action starts and did not treat step shell selection as executable authority. Both supported YAML forms execute on the maintained-main release path before construction, so the existing semantic boundary needed representation-complete fail-closed admission.
- Approved behavior or requirement IDs affected: `BEH-004`, `BEH-007`, `BEH-013`; `R-014`, `R-018`, `R-024`, `R-029`, `R-030`; `AC-006`, `AC-010`, `AC-025`, `AC-035`, `AC-036`; confirmed `MP-AR-025B`. Runtime/provider/model/Protocol/archive/assets/desktop behavior remains unchanged.
- Implementation delta:
  - The existing closure-bound private helper now parses the sole canonical workflow step list into complete step blocks. Every step must use a canonical block-map representation, select exactly one `run` or `uses` execution mechanism, and expose only reviewed fields. Flow-style/alias/quoted/noncanonical step forms reject instead of disappearing from enumeration.
  - The same helper now closes the complete ordered run surface by exact step IDs, conditions, continuation state, and environment maps, in addition to its exact action identity/input projection and package-manager command admission. Added run/action steps and executable field/environment drift reject.
  - Workflow/job defaults and per-step custom shell selection reject. The exact CRR-065 actionlint-valid flow-style `actions/github-script@v7` install and `bash -c 'npm install ... && bash {0}'` shell probes are durable negatives.
  - Non-executable workflow name/comment changes still project identically. The public facade, private helper closure binding, canonical package projection, focused-renewal outcome, and no-raw-hash/no-caller-validator boundaries remain intact.
- Changed files or areas: `build/workflow-executable-surface.mjs`, `tests/build/host-package-input-contract.test.mjs`; source commit `77092392ce565f887c4698a3a12f384ea41b5e02` after IR-041 source `447d34148500ef56aad94a62da215e7f3066e2b6`.
- Local validation and result: combined focused contract/closure/release/admission tests passed `18/18`; `npm run check:release-pipeline` passed `22/22`; exact pinned-Go full `npm run check` passed source guards, `7/7` Python plus compileall, all Go/source/evidence checks, and `112/112` Node TAP tests with no failures or skips. Prettier, diff/staged/commit checks, and source-size checks passed; public contract is 413 effective non-empty lines and private helper is 283.
- Next recipient or routing: `code_reviewer`.
- Remaining limitations or risks: implementation did not run API/E2E, construct/install production hosts, run offline clips or product qualification, compare Execution Closure 2, renew focused authorities, promote `R`, dispatch release, merge, tag, publish, or touch desktop/user state. API/E2E remains paused pending Source Pass; any later Execution Closure 2 mismatch routes to full qualification.
