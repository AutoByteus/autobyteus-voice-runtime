...
- Still-relevant prior source/API/Delivery records:
  ...
  - `/Users/normy/autobyteus_org/autobyteus-worktrees/voice-runtime-qualified-recovery/tickets/done/voice-input-runtime-reliability/code-review-evidence/CRR-048-on-demand-runtime-source-review.md`
  - `/Users/normy/autobyteus_org/autobyteus-worktrees/voice-runtime-qualified-recovery/tickets/done/voice-input-runtime-reliability/code-review-evidence/CRR-049-on-demand-runtime-resolution.md`
  - `/Users/normy/autobyteus_org/autobyteus-worktrees/voice-runtime-qualified-recovery/tickets/done/voice-input-runtime-reliability/code-review-evidence/CRR-050-api-f-016-f-017-origin.md`
...
`IR-033` resolves the two implementation composition failures exposed by `API-REV-022` and classified by `CRR-050`. The real Chinese Runtime Host Archive 2 builder now imports the resolved-CMake functions from their owning module while retaining the trusted environment under its existing owner. The canonical Go extractor now reports the validated logical archive root `host` instead of exposing its private absolute extraction destination. Production-shaped coverage loads both real builders and drives a real canonical archive through the Go extractor and Host Verification 2 owner. All `IR-032` catalog/store/lifecycle/resume corrections and the clean-cut SR-021 target remain unchanged.

- Implementation cycle: `Local Fix after source review`
...
- Current implementation revision: `IR-033`
...
- Related code reviews: `CRR-049` prior Pass; `CRR-050` Fail — Local Fix; source re-review pending
- Related API/E2E revision: `API-REV-022` Fail at host construction/verification prerequisites; earlier evidence retained
- Related Delivery revision: prior `DR-006`; current Delivery not started
- Triggering findings: `CR-F-044` / `API-F-016`; `CR-F-045` / `API-F-017`
...
| `CR-F-044`, `CR-F-045`; `API-VOICE-018`; `AC-028`                                                                                                   | Preserve owner boundaries while making real Chinese host construction and independent Host Verification 2 composition executable.                                                              | `build/profile-builders/funasr-host.mjs`; `build/resolved-cmake-configuration.mjs`; `packaging/archive/safeextract.go`; `tests/build/host-builder-composition.test.mjs`; `packaging/archive/canonicalzip_test.go`                  | Both real builders instantiate through their production imports. Resolved-CMake functions come from their sole owner. Extractor evidence projects `expected.Archive.RootDirectory`; the absolute destination remains private operational state.                                                         |
...
| `CR-F-039`–`CR-F-043`; Store 1, Catalog 4, Activation State Protocol 1                                                                              | Make the existing owners fail closed over exact current catalog authority, all store ancestors, signal identity, orphan lifetime, and resumable capacity.                                      | `contracts/{catalog,model}/current.go`; `modelmanager/internal/{catalog_validation,downloader,install,lifecycle}.go`; `modelstore/{safefs,safetree,verify,partials,prune}.go`                                                      | Exact two-row order and both subjects are validated before side effects; descriptor-relative operations reject nested symlink/special/hard-link aliases; signal/state are one atomic word; later writers prune only after lease/reference proof; remaining authenticated bytes drive capacity.          |
...
- Runtime/install authority: `hostverify/`, `integrity/`, `modelmanager/`, `modelstore/`, `launcher/internal/`, `contracts/{catalog,host,install,model,startup}/`; descriptor-relative Store 1 operations are centralized in `modelstore/safefs.go`.
...
- Reviewed change posture: `Bounded local hardening within the accepted clean-cut architecture`
- Reviewed root-cause classification: incomplete validation/atomicity/containment/lifetime handling inside the existing CatalogResolver, ModelInstallationService, Store 1, ActivationStore, and lifecycle owners.
- Reviewed refactor decision: `Refactor Needed Now inside the existing owners; no design reset`
...
- `VOICE_GO=/tmp/autobyteus-go1.26.5-sr021-v2/go/bin/go npm run check` — passed source guards, `7/7` Python tests plus compileall, all Go tests/guards, English-v2 and Chinese-v2 evidence verification, and `93/93` Node TAP tests.
- Focused `VOICE_GO=... node --test tests/build/host-builder-composition.test.mjs` — passed `2/2`, including both real builder module graphs and real canonical Go build/extract through Host Verification 2.
- Trusted-Go `go vet ./...` — passed.
- Trusted-Go `go test -race ./...` — passed across all Go packages, including the direct logical-root/private-destination extractor assertion.
- Trusted-Go focused `go test -race ./modelmanager/internal ./modelstore ./launcher/internal` and repeated `go test -count=10 ./modelmanager/internal ./modelstore` — passed.
- Prettier check for the changed Catalog 4 schema/release sources and `git diff --check` — passed.
...
1. Resume API/E2E at exact Chinese double construction, then independently verify both English and Chinese real archives; require logical `host` evidence and byte-identical reports/archives before later scenarios.
2. Exercise fresh and partial production-manifest installs where disk capacity is enough only for authenticated remaining bytes plus metadata and 64 MiB; cover valid resume, stale-validator restart, and completed-file reuse against real CDN behavior.
3. Prove complete ordered Catalog 4, host verification, and admission rejection occur before caller notice/manifest reads, network access, or store mutation for non-selected-row and selected-row authority drift.
...
5. On macOS, race nested store ancestor replacement, SIGINT/SIGTERM acceptance, pointer replacement, provider leases, crash/precommit cleanup, remove, and bounded status snapshots; assert no outside-root mutation and no referenced subject pruning.
6. Derive and independently verify Profile Execution Closure 2, Focused Qualification Set 3, and Branch Catalog Projection 3; route any closure difference to the prescribed qualification scope.
7. Execute the standard-hosted release only after source/API/E2E/code-review gates pass; verify exact nine published bytes, downloaded-byte identities, and tag-preserving quarantine behavior.
