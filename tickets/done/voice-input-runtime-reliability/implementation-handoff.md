...
`IR-032` resolves all five bounded findings from `CRR-048` without changing the clean-cut SR-021 target. Catalog resolution now validates the complete exact ordered two-row Catalog 4 and fully binds the selected row before store or network access. Store 1 descendant mutation and verification use descriptor-relative, no-follow operations; accepted cancellation publishes signal and state atomically; later writers prune only unreferenced crash/precommit and replaced subjects after lease/reference proof; and resume capacity uses authenticated remaining bytes plus metadata and the fixed 64 MiB reserve. The model-free hosts, Session Config 2, provider lifetime lease, exact nine-asset hosted chain, and legacy removal remain unchanged.
...
- Current implementation revision: `IR-032`
...
- Related code reviews: `CRR-048` Fail — Local Fix; source re-review pending
...
- Triggering findings: `CR-F-039`, `CR-F-040`, `CR-F-041`, `CR-F-042`, `CR-F-043`
- Source commit: `ad7c402d224690584e2da98ec71a73e8b6d4ca36`
...
- Trusted-Go `go test -race ./...` — passed across all Go packages, including new Catalog 4, nested-store-ancestor, signal race, orphan-pruning, and resume-capacity regressions.
