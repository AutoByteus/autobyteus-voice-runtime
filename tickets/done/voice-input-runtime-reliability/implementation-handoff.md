...
`IR-028` is the bounded `CRR-042` Local Fix against `SR-018` / `ARCH-REV-019`. The singular Aggregate API Renewal authority owner now independently resolves and binds every semantic subject needed by candidate promotion: the accepted record commit, reviewed source/test/promotion commit lineage, coverage-report Git blob and content, retained archive/profile evidence, and current/prior aggregate identities plus byte-comparison flags. Candidate assembly derives those subjects from independently verified candidate inputs and uses only the authority owner's verified reference.
...
- Source commit: `bbfa803f5b6126635c73e778fb81e0c6acb631f0`
- Prior implementation artifact HEAD: `8c9c149980516bcc23a51e1e05c5bed792d02949`
...
- Added 12 schema-valid negative cases plus a real temporary-Git-repository success case covering exact record/report/profile/lineage resolution.

## Important Assumptions

- The accepted Profile/Qualification authority remains commit `32829080938911f0f46390a3fd2af823e105bd32`.
- Aggregate renewal remains a later zero-profile-execution API/E2E operation, committed before a separate reviewed policy/controller commit. IR-028 validates that eventual chain but does not fabricate or accept it.
...
- Changed implementation file limits: both changed source files remain below 500 effective non-empty lines. `release/candidate-authority.mjs` exceeds the 220-line delta signal because the reviewed design requires one singular aggregate-authority owner; splitting its validation into a second authority/parser would violate the boundary. Its concerns remain subordinate verification steps within that owner.
...
- IR-028 changes verification only and does not migrate, rewrite, or accept any retained record.
- Deviation: `None`.
...
- `node --test tests/release/qualified-release-candidate.test.mjs` — pass: 27/27 tests.
- `npm run check:release-pipeline` — pass: 45/45 tests plus release source/schema guards.
- `PATH=/tmp/autobyteus-go1.26.5-ir027/go/bin:$PATH VOICE_GO=/tmp/autobyteus-go1.26.5-ir027/go/bin/go npm run check` — pass after source commit: source guards; 7/7 Python plus compileall; all Go tests/guards; English-v2 and Chinese-v2 evidence verification; 155/155 Node TAP tests.
...
Yes. API/E2E remains paused until Code Review passes IR-028. The next authorized downstream operation is focused zero-profile Aggregate API Renewal, not archive recovery or profile qualification.
