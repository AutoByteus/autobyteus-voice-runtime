...
- Implementation Handoff: `/Users/normy/autobyteus_org/autobyteus-worktrees/voice-runtime-qualified-recovery/tickets/done/voice-input-runtime-reliability/implementation-handoff.md` (`IR-033`)
...
- Code Review Report: `/Users/normy/autobyteus_org/autobyteus-worktrees/voice-runtime-qualified-recovery/tickets/done/voice-input-runtime-reliability/code-review-report.md` (`CRR-051 Pass`)
...
- Current API/E2E Revision ID: `API-REV-023`
- Current Investigation Round: `23`
- Trigger: Code Reviewer `CRR-051` against exact source commit `4db8bf26708309440c83ec56973250f77e9f1619` and reviewed artifact/HEAD `bd70e942dd6ed3b49d7db5221dfe13f14b44032f`.
- Prior Investigation Reviewed: `API-REV-022 — Fail / 84%`; `API-F-016` and `API-F-017` are the mandatory first rechecks.
- Latest Authoritative Investigation: **`API-REV-023 — Fail / 84%`**. The repository correction passes its focused and full gates and directly resolves the prior import failure, but the first canonical current-source Chinese construction exposes `API-F-018`: the inner builder rejects a complete authenticated manifest path that the outer assembler is designed to consume. No archive is created, so downstream execution remains fail closed.

## API-REV-023 Recheck And Coverage Delta

- `API-F-016` recheck: execute the real Chinese builder twice from exact current source, compare archives and reports byte-for-byte, and independently verify both products.
- `API-F-017` recheck: independently verify freshly built English and Chinese hosts and require logical `hostRoot: "host"` with no private extraction destination in the public report.
- Retained environment decision: `Reusable`. API-REV-022 authenticated cache objects, materialized immutable external inputs, official CMake 4.2.0, official Go 1.26.5, Xcode/SDK/tool identities, and isolated ownership remain exact. They will be revalidated before use.
- Retained English archive decision: `Do Not Relabel / Rebuild`. The package-relevant source bytes appear unchanged, but its retained build report and reproducibility proof truthfully bind source `ad7c402...`; current API-REV-023 authority must bind `4db8bf2...`.
- Durable coverage decisions:
  - `tests/build/host-builder-composition.test.mjs`: `Still Valid / Added Upstream`. It now imports both production builder graphs and drives a real canonical archive through Host Verification 2.
  - `packaging/archive/canonicalzip_test.go`: `Still Valid / Updated Upstream`. It asserts exact logical host root and destination non-disclosure.
  - No API/E2E-owned durable coverage edit is planned unless realistic execution exposes a distinct uncovered boundary.
- Ordered execution: authenticate retained environment and repository state; run focused and full repository gates; rebuild/verify both hosts; only on Pass continue production Catalog 4/model installation, realistic Store 1 lifecycle/interleavings, relocated offline retained-clip provider smoke, Profile Execution Closure 2, Focused Qualification Set 3, Branch Catalog Projection 3, and nonpublishing nine-member composition.
- Stop rule: any critical construction, verification, production installation, provider, or closure mismatch stops downstream aggregation and is recorded fail closed without threshold/provider/model substitution.

### API-REV-023 Execution Update

- Environment/input proof: retained build-environment SHA-256 remains exact at `1ffed2538d3d65c1fbb62d319e7d66d27b840652eca8cb524ae876a68467bf2a`; Go 1.26.5, Node 22.23.1, CMake 4.2.0, Xcode/SDK identities, AC power, and capacity recheck Pass. Both profile inputs rematerialize against source `4db8bf2...` and both current Host Source Closure 1 records derive successfully.
- Repository proof: corrected focused builder/verifier coverage 2/2 Pass; archive race Pass; release pipeline 9/9 Pass; full gate 93/93 Node TAP, 7/7 Python plus compileall, and all Go/source/evidence checks Pass. One first focused invocation without `VOICE_GO` is retained as an excluded setup attempt and rerun correctly.
- `API-F-016`: resolved at its exact import boundary. The production Chinese builder loads and reaches its input-consumer closure.
- `API-F-017`: focused real archive extraction now projects logical `host` and passes. Full packaged-host verification remains not tested because construction fails first.
- `API-F-018`: new `Local Fix`. `assertHostInputClosure()` in the Chinese builder rejects `host-authority/model-admission-root-v1.json`, although `host-package-assembler.mjs` intentionally consumes that authority after the profile builder returns. The sibling compatibility authority would be subject to the same ownership mismatch. This is a real production composition defect not exercised by the newly added module-instantiation test.
- Stop/cleanup: the first Chinese build exits before CMake and creates no archive. English current-source construction, model downloads, Store 1 mutation, providers, Profile Execution Closure 2, QSet 3, Projection 3, and release composition are Not Tested after the critical `AC-028` failure. No owned process remains.
...
| `tests/build/host-builder-composition.test.mjs`                                                                                                                         | imports both production builders and drives a small real archive through Host Verification 2 | `AC-028`                              | Needs Update                                      | 2/2 Pass, but canonical Chinese construction fails on a complete production manifest at API-F-018 | add a production-shaped complete-input ownership assertion during source correction |
