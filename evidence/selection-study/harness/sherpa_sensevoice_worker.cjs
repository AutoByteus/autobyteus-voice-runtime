...
// Disposable controlled sherpa-onnx SenseVoice worker used only by the backend study.
...
function parseArgs(values) {
  ...
  for (const key of ['sherpa-package', 'model', 'tokens', 'language']) if (!parsed[key]) throw new Error(`Missing --${key}`)
...
async function main() {
  ...
  emit({
    type: 'ready', backend: 'sherpa-onnx-sensevoice-int8', pid: process.pid,
    ...
    while ((newline = buffer.indexOf('\n')) !== -1) {
      ...
      try {
        ...
        emit({
          ...
          text: String(result.text || '').trim(), detectedLanguage: result.lang || result.language || args.language,
