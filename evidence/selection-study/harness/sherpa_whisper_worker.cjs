...
// Disposable controlled sherpa-onnx Whisper worker used only by the backend study.
...
function parseArgs(values) {
  ...
  for (const key of ['sherpa-package', 'encoder', 'decoder', 'tokens', 'language', 'precision']) {
...
async function main() {
  ...
  const recognizer = await sherpa.OfflineRecognizer.createAsync({
    ...
    modelConfig: {
      whisper: {
        ...
        language: args.language === 'auto' ? '' : args.language,
        task: 'transcribe',
      },
      tokens: args.tokens,
  ...
  emit({
    type: 'ready',
    backend: `sherpa-onnx-whisper-${args.precision}`,
    pid: process.pid,
    importMs: round(importMs),
    modelLoadMs: round(performance.now() - modelStarted),
    workerReadyMs: round(performance.now() - PROCESS_STARTED),
  ...
  async function transcribe(request) {
    ...
    try {
      ...
      emit({
        ...
        detectedLanguage: result.lang || result.language || args.language,
        readMs: round(readMs), decodeMs: round(decodeMs),
        transcribeMs: round(performance.now() - requestStarted),
        rssBytes: process.memoryUsage().rss,
      })
    } catch (error) {
      emit({
