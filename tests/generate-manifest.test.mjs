...
test('generate-manifest emits lightweight release bundle metadata', async () => {
  ...
  await execFileAsync('node', [scriptPath, outputPath], {
    env: {
      ...process.env,
      AUTOBYTEUS_VOICE_RUNTIME_VERSION: '0.3.0',
      AUTOBYTEUS_RELEASE_REPOSITORY: 'AutoByteus/autobyteus-voice-runtime',
      AUTOBYTEUS_RELEASE_TAG: 'v0.3.0',
  ...
  assert.equal(
    ...
    'https://github.com/AutoByteus/autobyteus-voice-runtime/releases/download/v0.3.0/voice-input-runtime-darwin-arm64.tar.gz',
  )
  assert.match(manifest.assets[0].sha256, /^[a-f0-9]{64}$/)
  assert.equal(manifest.assets[0].backendKind, 'mlx')
  assert.equal(manifest.assets[0].model.sourceRepo, 'mlx-community/whisper-small-mlx')
  assert.equal(manifest.assets[0].model.sourceRevision, 'main')
  assert.equal(manifest.assets[0].model.version, 'mlx-community/whisper-small-mlx')
  assert.equal(manifest.assets[0].model.url, undefined)
})
